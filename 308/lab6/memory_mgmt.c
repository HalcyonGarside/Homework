 ...
 * num_frames = how many frames in your table
...
int PRAlgo_FIFO(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	int earliest_time = 999999999;
	for(int i = 0; i < num_frames; i++)
	{
		PageFrame cur_frame = PageFrames[i];
		if(cur_frame.time_of_arrival < earliest_time)
		{
			earliest_time = cur_frame.time_of_arrival;
			frame_to_evict = i;
		}
	}
...
int PRAlgo_LRU(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	int earliest_time = 99999999;
	for(int i = 0; i < num_frames; i++)
	{
		PageFrame cur_frame = PageFrames[i];
		if(cur_frame.time_of_access < earliest_time)
		{
			earliest_time = cur_frame.time_of_access;
			frame_to_evict = i;
		}
	}
