...
void *producer(void *arg)
{
  ...
  while (!producer_done)
  {
	//Wait for either the supply or the number of consumers
	//to be exhausted
	while(supply > 0 && num_cons_remaining > 0)
		pthread_cond_wait(&producer_cv, &mut);

	//If the number of consumers is exhausted, exit
	if(num_cons_remaining == 0)
	{
		producer_done = 1;
	}
	//If there are still consumers, resupply and call all consumers
	else
	{	
