...
void decodeBootSector(struct BootSector * pBootS, unsigned char buffer[])
{
	...
	pBootS->iSectorsTrack = endianSwap(buffer[0x18], buffer[0x19] << 8);
	pBootS->iHeads = buffer[0x1A];

	// TODO: Read hidden sectors	
	pBootS->iHiddenSectors = buffer[0x1C];
}
...
void parseDirectory(int iDirOff, int iEntries, unsigned char buffer[])
{
	...
	int oset = (i * 32);
		...
		//Display filename
		toDOSName(string, buffer, oset);
		printf("%-12s\t", string);
		
		//Display Attributes
		parseAttributes(string, buffer[oset + 0x0b]);
		printf("%s\t", string);
		
		//Display Time
		parseTime(string, endianSwap(buffer[oset + 0x16], buffer[oset + 0x17]));
		printf("%s\t", string);
		
		//Display Date
		parseDate(string, endianSwap(buffer[oset + 0x18], buffer[oset + 0x19]));
		printf("%s\t", string);
		
		//Display Size
		printf("%d\n", endianSwap(buffer[oset + 0x1c], endianSwap(buffer[oset + 0x1d], endianSwap(buffer[oset + 0x1e], buffer[oset + 0x1f]))));
    }
